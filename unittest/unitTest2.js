...
describe('Decoder', function(){
	...
	it('decode the message "THIS IS A HOAX" accurately, regular input', // base test case
	function(){
        var msg = "THIS IS A HOAX";
		var decoder = new Decoder(msg, 1);
		expect(decoder.morseSimulator(msg)).to.equal(msg);
	});
    it('Detecting negative gap times and exiting early', // testing for non positive gap times
	function(){
    ...
    it('Returns null when a signal does not match any entries in the morse code table', // test non matching signals
	function(){
		var decoder = new Decoder("", 1);
  ...
  it('Exits early the moment an SK is detected and returns the message that has been decoded so far', // early exits after decoding the SK char
	function(){
		var decoder = new Decoder("", 1);
